...
class Scene1 extends Phaser.Scene {
  ...
  create() {
    ...
    const springHolder = this.matter.add.rectangle(
      width / 2,
      height / 18 / 2,
      width,
      height / 18,
    ...
    this.spring1 = this.matter.add.spring(
      springHolder,
      circle1,
      height * 0.2,
      0.001,
      { pointA: { x: -width / 2 + width / 6, y: 0 } }
    );
    this.spring2 = this.matter.add.spring(
      springHolder,
      circle2,
      height * 0.3,
      0.001,
      { pointA: { x: -width / 2 + width / 3, y: 0 } }
    );
    this.spring3 = this.matter.add.spring(
      springHolder,
      circle3,
      height * 0.4,
      0.001,
      { pointA: { x: -width / 2 + width / 2, y: 0 } }
    );
    this.spring4 = this.matter.add.spring(
      springHolder,
      circle4,
      height * 0.5,
      0.001,
      { pointA: { x: -width / 2 + width / 1.5, y: 0 } }
    );
    this.spring5 = this.matter.add.spring(
      springHolder,
      circle5,
      height * 0.6,
      0.001,
      { pointA: { x: -width / 2 + width / 1.2, y: 0 } }
    );
  ...
  update() {
    const length1 = Phaser.Math.Distance.Between(
      this.spring1.pointA.x,
      this.spring1.pointB.y,
      this.spring1.bodyB.position.x,
      this.spring1.bodyB.position.y
    );
    const length2 = Phaser.Math.Distance.Between(
      this.spring2.bodyA.position.x,
      this.spring2.bodyA.position.y,
      this.spring2.bodyB.position.x,
      this.spring2.bodyB.position.y
    );
    const length3 = Phaser.Math.Distance.Between(
      this.spring3.bodyA.position.x,
      this.spring3.bodyA.position.y,
      this.spring3.bodyB.position.x,
      this.spring3.bodyB.position.y
    );
    const length4 = Phaser.Math.Distance.Between(
      this.spring4.bodyA.position.x,
      this.spring4.bodyA.position.y,
      this.spring4.bodyB.position.x,
      this.spring4.bodyB.position.y
    );
    const length5 = Phaser.Math.Distance.Between(
      this.spring5.bodyA.position.x,
      this.spring5.bodyA.position.y,
      this.spring5.bodyB.position.x,
      this.spring5.bodyB.position.y
    );
